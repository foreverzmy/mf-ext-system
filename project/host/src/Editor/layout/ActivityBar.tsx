...
export const ActivityBar: FC = memo(() => {
  ...
  return (
    <div
      className="h-full shadow shadow-slate-300 border-r border-slate-200"
      style={{ width: '56px', minWidth: '56px', marginRight: '1px' }}
    >
      {activitybars.map(menu => (
        <div
          key={menu.id}
          className="flex flex-col justify-center items-center w-full cursor-pointer hover:bg-amber-100"
          style={{
            height: '72px',
            padding: '3px',
            margin: '10px 0',
            background: current === menu.id ? 'aquamarine' : '',
          }}
